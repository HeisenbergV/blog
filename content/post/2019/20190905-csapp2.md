...
### 消除不必要的内存引用
combine3将计算值累积在指针dest位置，这个操作会进行三步操作：1. 读取dest值 2. 读取data[i]值 3. 写入dest所在内存
从dest读取的值，就是上次写入dest的值。所以可以优化：1. 读data[i]值，计入寄存器中
最后只需要读取一次data[i]值即可。
...
```
这样看来，编译器应该也会帮我们优化成combine4的形式。但由于内存别名的使用，两个函数可能会有不同行为，
