...
`fork`函数： 创建的新进程被称为子进程，子进程的内存内容同创建它的进程（父进程）一样。fork 函数在父进程、子进程中都返回（一次调用两次返回）。对于父进程它返回子进程的 pid，对于子进程它返回0。
所以下面代码`fork()`后，一共两个进程，每个进程的执行内容一模一样，但fork的返回结果不同，一个parent一个child，父的fork返回不为0进入else语句，子进程相反。

`pipe`函数：如果数据没有准备好，那么对管道执行的read会一直等待，所以parent和child进程在没有获取到对方数据时不会`exit()`，因为parent是先写，child是先读，所以程序执行后的输出顺序总是先输出child的log
```c
#include "kernel/types.h"
#include "user/user.h"
